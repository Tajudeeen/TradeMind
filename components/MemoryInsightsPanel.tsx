...
export default function MemoryInsightsPanel({ memory, isLoading, isInitializing }: MemoryInsightsPanelProps) {
    ...
    <div className="h-full flex flex-col gap-3 p-4 overflow-y-auto">
      ...
      {showExport && (
        <div className="bg-zinc-900/80 border border-zinc-700/50 rounded-xl p-3 flex flex-col gap-2">
          <p className="text-xs text-zinc-400">Download your full memory as JSON. You can inspect everything the agent knows about you.</p>
          <button
      ...
      <div className={`rounded-xl border p-3 ${RISK_BG[memory.risk_profile] || RISK_BG.unknown}`}>
        ...
        {memory.risk_profile !== "unknown" && (
          <div className="text-xs text-zinc-600 mt-0.5">
            Based on {memory.user_behavior.reduce((s, b) => s + b.count, 0)} signals
          </div>
        )}
      </div>
            ...
            {topBehaviors.map((b) => (
              <div key={b.tag} className="bg-zinc-900/60 border border-zinc-800/50 rounded-lg px-3 py-2">
                ...
                <div className="text-xs text-zinc-700 mt-0.5">
                  Last seen {new Date(b.last_seen).toLocaleDateString("en-US", { month: "short", day: "numeric" })}
                </div>
              </div>
            ))}
          </div>
      ...
      {memory.last_updated && (
        <div className="text-xs text-zinc-700 text-center mt-auto pt-2">
          Last updated {new Date(memory.last_updated).toLocaleTimeString("en-US", { hour: "2-digit", minute: "2-digit" })}
        </div>
      )}
    </div>
  );
